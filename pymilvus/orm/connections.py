...
import threading
import time
from typing import Awaitable, Callable, Dict, Generic, List, Tuple, TypeVar, Union
...
NoneT = TypeVar("NoneT", None, Awaitable[None])
GrpcHandlerT = TypeVar("GrpcHandlerT", bound=GrpcHandler)  # TODO: use BaseGrpcHandler


class BaseConnections(Generic[GrpcHandlerT, NoneT], metaclass=SingleInstanceMetaClass):
    ...
    def __init__(self) -> None:
        ...
        self._connected_alias: Dict[str, GrpcHandlerT] = {}
        self._env_uri = None
    ...
    def disconnect(self, alias: str) -> NoneT:
    ...
    def connect(
        ...
        **kwargs,
        ...
        # Make sure passed in None doesn't break
        ...
        if with_config(config):
            ...
            return self._connect(
                alias, kwargs_copy, **kwargs, user=user, password=password, token=token, db_name=db_name
            )
        ...
        if alias in self._alias:
            ...
            return self._connect(
                alias, kwargs_copy, **connect_alias, password=password, db_name=db_name, **kwargs
            )
    ...
    def remove_connection(self, alias: str):
        ...
        self._disconnect(alias, remove_connection=True)
    ...
    def __get_full_address(
        ...
        host: str = "",
        port: str = "",
    ) -> Tuple[str, Union[parse.ParseResult, None]]:
        if address != "":
        ...
        if not is_legal_address(addr):
            raise ConnectionConfigException(
                message=f"""
                    Illegal host: {host} or port: {port},
                    should be in form of '111.1.1.1', '19530'
                """
            )

        return addr, None

    def _fetch_handler(self, alias: str = Config.MILVUS_CONN_ALIAS) -> GrpcHandlerT:
    ...
    def _connect(self, alias: str, kwargs_copy: Dict, **kwargs) -> NoneT:
        raise NotImplementedError


class Connections(BaseConnections[GrpcHandler, None]):
    """Sync Connections."""
    ...
    def _connect(self, alias: str, kwargs_copy: Dict, **kwargs):
